...
import 'package:flutter_sample/pullrefresh/refresh/magic_activity_indicator.dart';
import 'package:flutter_sample/pullrefresh/refresh/magic_refresh.dart';

class CustomRefreshDemo extends StatefulWidget {
  const CustomRefreshDemo({Key? key}) : super(key: key);
...
class _CustomRefreshDemoState extends State<CustomRefreshDemo> {
  GlobalKey<MagicSliverRefreshControlState> key =
      GlobalKey<MagicSliverRefreshControlState>();
  ...
  Widget buildRefreshIndicator(
    ...
    final double percentageComplete2 =
  ...
  Widget _buildIndicatorForRefreshState(RefreshIndicatorMode refreshState,
    ...
    print(
        "<> _buildIndicatorForRefreshState $refreshState  $radius $percentageComplete");
    switch (refreshState) {
      case RefreshIndicatorMode.drag:
        const Curve opacityCurve = Interval(0.0, 0.35, curve: Curves.easeInOut);
        return Opacity(
          opacity: opacityCurve.transform(percentageComplete),
          child: MagicActivityIndicator.partiallyRevealed(
