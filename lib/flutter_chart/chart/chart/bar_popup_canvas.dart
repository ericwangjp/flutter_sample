...
import 'package:flutter_sample/flutter_chart/chart/canvas/base_chart_canvas.dart';
...
import 'package:flutter_sample/flutter_chart/chart/common/text_element.dart';

/// 绘制Bar Popup悬浮框
/// [BaseLayoutConfig.popupSpec]
class BarPopupCanvas {
  static void draw<T>({
    ...
    switch (config.gestureDelegate!.type) {
      ...
      case GestureType.onTapDown:
        ...
        /// 修正绘制区域
        double top;
        var verticalSpacing = delegate?.labelVerticalSpacing ?? 0;
        var barHeight = delegate?.barStyle?.height ?? 0;
        var spacing = verticalSpacing + barHeight;
        var dis = find.offset.dy - rectHeight - spacing;
        ...
        var strokeWidth = delegate?.hintLineStyle?.strokeWidth ?? 0;
        var points = [
        ...
        if (popup.bubbleShouldDraw?.call(find.model) ?? true) {
          var yOffset = Offset(0, (delegate?.barStyle?.height ?? 0) / 2);
          chartCanvas.drawPoint(
            canvas: canvas,
            offset: find.offset + yOffset,
        ...
        /// 绘制Popup
        if (true == popup.popupShouldDraw?.call(find.model)) {
