...
class _BezierChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void _drawBezierLinePath(BezierLine line) {
      ...
      /// 绘制折线下方区域
      if (config?.areaGradientColors != null) {
        var shader = LinearGradient(
                begin: Alignment.topCenter,
                end: Alignment.bottomCenter,
                tileMode: TileMode.clamp,
                colors: (config?.areaGradientColors)!)
            // .createShader(Rect.fromLTRB(0, size.width, 0, 0));
            .createShader(path.getBounds());

        // 从path的最后一个点连接起始点，形成一个闭环
        shadowPath.addPath(path, Offset.zero);
        shadowPath
          ..lineTo(path.getBounds().right, height + line.lineStrokeWidth / 2)
          ..lineTo(path.getBounds().left, height + line.lineStrokeWidth / 2)
          ..close();
    ...
    if (verticalIndicatorPosition != null && showIndicator!) {
      ...
      if (p0 != null) {
        ...
        //base value, modified based on the label text
        double infoWidth = 0;
        ...
        canvas.drawCircle(
          ...
          Paint()
            ..style = PaintingStyle.stroke
            ..color = series!.reversed.toList().last.dataPointStrokeColor
            ..strokeWidth = config?.selectedPointStrokeWidth ?? 4.0,
        );
          ...
          textValues.add(
            TextSpan(
              text: config!.bubbleIndicatorValueFormat != null
                  ...
                  : "${customValue.value} ",
              style: config!.bubbleIndicatorValueStyle.copyWith(fontSize: 16),
              children: [
                TextSpan(
